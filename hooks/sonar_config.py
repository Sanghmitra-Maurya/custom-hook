...
class SonarConfig:
    ...
    def _get_token(self, config: Dict[str, str], env_token: str) -> bytes:
        """Get SonarQube token from config or environment, return encrypted."""
        from .setup_details import encrypt_token
       
        # Try to find token in config first
        for token_key in self.TOKEN_KEYS:
            if token_key in config:
                # Encrypt the token from properties file
                return encrypt_token(config[token_key])
       
        # Fall back to environment token (already encrypted)
        self.defaults_used.append("sonar.token -> encrypted token")
