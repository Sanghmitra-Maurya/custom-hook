...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    for filename in args.filenames:
        ...
        if args.check_only:
            # Check if file has trailing whitespace without fixing
            with open(filename, mode='rb') as file_processed:
                lines = file_processed.readlines()
            newlines = [_process_line(line, md, chars) for line in lines]
            if newlines != lines:
                print(f'File has trailing whitespace: {filename}')
                return_code = 1
        else:
