...
class SonarQubeCheck:
    ...
    def run_analysis(self):
        ...
        try:
            auth_param = "-Dsonar.token" if self.sonar_config.get('is_token', True) else "-Dsonar.login"
            
            cmd = [
                "sonar-scanner.bat",
                f"-Dsonar.projectKey={self.project_key}",
                f"-Dsonar.host.url={self.sonar_host}",
                f"{auth_param}={self._get_auth_token()}"
            ]
...
def main():
    ...
    sonar_config = config if language == "project-default" else {'is_token': config.get('is_token', True)}
